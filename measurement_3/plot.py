...
def plot_data(ax, data_files, title):
    for file_name in data_files:
        ...
        with open(file_name, "r") as file:
            for line in file:
                ...
                if x < 31:  # Filter data points with x < 31
                    x_values.append(x)
                    y_values.append(y)
    ...
    ax.set_xlabel("Hops", fontsize=12)
    ax.set_ylabel("Probability", fontsize=12)
    ax.legend()
    ax.grid(True, linestyle='--', alpha=0.7)

    # Set the x-axis and y-axis ticks
    x_ticks = np.arange(min(x_values), max(x_values) + 1, 1)  # Adjusted tick difference for smaller range
